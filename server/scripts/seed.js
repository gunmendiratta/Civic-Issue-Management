...
if (process.env.ADMIN_EMAIL && process.env.ADMIN_PASSWORD) await User.updateOne({ email: process.env.ADMIN_EMAIL.toLowerCase() }, { $set: { name: process.env.ADMIN_NAME || 'CivicConnect Administrator', passwordHash: await bcrypt.hash(process.env.ADMIN_PASSWORD, 12), role: 'admin', active: true }, $setOnInsert: { email: process.env.ADMIN_EMAIL.toLowerCase() } }, { upsert: true })
